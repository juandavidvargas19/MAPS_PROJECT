...
class SecondOrderNetwork(nn.Module):
    ...
    def forward(self, comparison_matrix, prev_comparison, cascade_rate):
        ...
        # Normalize inputs using Min-Max Scaling along the feature dimension (dim=1)
        '''
        min_val = torch.min(comparison_matrix, dim=1, keepdim=True)[0]
        max_val = torch.max(comparison_matrix, dim=1, keepdim=True)[0]
        comparison_matrix = (comparison_matrix - min_val) / (max_val - min_val + 1e-5)  # Avoid division by zero
        '''
        
        # Clamp input values to prevent extreme values
        #comparison_matrix = torch.clamp(comparison_matrix, min=-1e2, max=1e2)


        # Check for NaN values after normalization and clamping
        #if torch.any(torch.isnan(comparison_matrix)):
        #    print("NaNs detected before comparison layer!")

        # Pass the normalized input through the comparison layer and apply dropout and activation
        ...
        # Check for NaN values after comparison layer and dropout
        #if torch.any(torch.isnan(comparison_out)):
        #    print("NaNs detected after comparison layer and dropout!")
        
        
        # Check for NaN values after activation
        #if torch.any(torch.isnan(comparison_out)):
        #    print("NaNs detected after activation!")
        
        # Calculate the wager value using softmax on wager layer output
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        if previous_loss!=None:
            ...
            with torch.no_grad():
                ...
                for j in range(cascade_iterations_1):
                    output_DQN_policy_teacher , h1_teacher , comparison_1_teacher, main_task_out_teacher = teacher_first_net(states, main_task_out_teacher, cascade_rate_1) 
                
            Q_s_a_teacher = output_DQN_policy_teacher.gather(1, actions)

            
        #print(output_DQN.size(),"size output DQN")
        ...
        if len(none_terminal_next_states) != 0:
            ...
            for k in range(cascade_iterations_1):
                output_DQN_target, h2, comparison_2, target_task_out = target_net(none_terminal_next_states, target_task_out, cascade_rate_1)
        ...
        ce_loss_weight=WEIGHT1

        soft_target_loss_weight=WEIGHT2
        feature_weight=WEIGHT3
        
        if previous_loss!=None and (not(evaluation_continuous_learning)):
            
            #loss_previous_task = distillation_loss(output_DQN_policy, output_DQN_policy_teacher)
            #loss_previous_task = distillation_loss(Q_s_a, Q_s_a_teacher)
            #loss_previous_task = CAE_loss(W, Q_s_a_teacher, Q_s_a, h1_teacher, lam)
            #loss_previous_task=f.mse_loss( h1 , h1_teacher) 
            loss_previous_task=compute_weight_regularization(policy_net, teacher_first_net)
                                          
            #feature_loss=compute_weight_regularization(policy_net, teacher_first_net)
        ...
        if meta:
            ...
            if previous_loss!=None and (not(evaluation_continuous_learning)):
                ...
                with torch.no_grad():
                    for i in range(cascade_iterations_2):
                        output_second_teacher, comparison_out_teacher = \
                ...
                #previous_task_loss_second = distillation_loss(output_second, output_second_teacher)
                
                task_loss_second=f.binary_cross_entropy_with_logits(output_second , targets_wagering)
                                
                #feature_loss_second=compute_weight_regularization(second_order_net, teacher_second_net)
                ...
                #loss_second= f.binary_cross_entropy_with_logits(output_second , targets_wagering)  #works with no activation layer at the end
                loss_second= f.binary_cross_entropy_with_logits(output_second, targets_wagering)
                

            if train_or_test == True: 
                #print("sizes",targets_wagering.size(),output_second.size())
...
def evaluation(env, meta, t, target_off, target_net, alpha, cascade_iterations_1, cascade_iterations_2, replay_start_size, num_actions, replay_off, r_buffer):
    ...
    loss_one_distillation=None
    loss_one_feature=None
    loss_one_task=None
    loss_two_distillation=None
    loss_two_feature=None
    loss_two_task=None
...
def dqn(env, replay_off, target_off, output_file_name, meta, alpha, cascade_iterations_1, cascade_iterations_2, game_name, curriculum=False, curriculum_evaluation=False, store_intermediate_result=True, load_path=None, checkpoints=False, adapnet=False):
    ...
    global list_losses_first, counter_list_losses, list_losses_first_distillation, list_losses_first_features ,  list_losses_second, list_losses_second_distillation, list_losses_second_features
    ...
    if load_path is not None and isinstance(load_path, str):
        ...
        try:
            # First attempt: Try loading with the safe globals
            ...
            try:
                ...
                # Third attempt: Try loading with pickle
        ...
        #STEP_SIZE=step_size1*LAMBDA
        #STEP_SIZE_SECOND=step_size2*LAMBDA
    ...
    if load_path is not None and isinstance(load_path, str):
        ...
        def load_partial_state_dict(model_original, state_dict):
            ...
            for name, param in state_dict.items():
                #print(f"Layer: {name}, Shape of weights: {param.shape}")
        ...
        #    print("starting evaluation, loaded first order")
        if not adapnet:
    ...
    while ((t < (NUM_FRAMES -reduce_curriculum) ) and (not is_curriculum_terminated)) or (curriculum_evaluation and (evaluation_curriculum_counter < number_validation_curriculum) ):
        ...
        while(not is_terminated) and t < (NUM_FRAMES-reduce_curriculum) and (not curriculum_evaluation):
            ...
            #if t%1000==0:
            #print("frame number ",t , "episode",e)
        ...
        if e % checkpoint_iteration == 0 or curriculum_evaluation:
            ...
            for _ in range(validation_iterations):
                ...
                if curriculum_evaluation:
                    # Call the evaluation function with the generated random number
                ...
                # Store individual results for std calculation
            ...
            if curriculum_evaluation:
                # Print and log results with means and standard deviations
                if meta: 
                    result_string = (
...
def main():
    ...
    #dict1_path = "/home/juan-david-vargas-mazuera/ICML-RUNS/WorkshopPaper/REPO/MinAtar/space_invaders_setting1_steps55000_501_data_and_weights"
    #dict2_path = "/home/juan-david-vargas-mazuera/ICML-RUNS/WorkshopPaper/REPO/MinAtar/space_invaders_setting1_steps45000_502_data_and_weights"

    #compare_saved_dicts(dict1_path, dict2_path)
