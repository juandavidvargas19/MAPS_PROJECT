...
class MettaAgent(nn.Module):
    def __init__(
        ...
        **cfg,
        ...
        print("COMPONENTS  ",component_cfgs)
    ...
    def forward(
        self, x: torch.Tensor, state: PolicyState, prev_hidden: Optional[torch.Tensor] = None, 
        cascade_rate: float = 1.0, action: Optional[torch.Tensor] = None
        ...
        Args:
            ...
            prev_hidden: Previous hidden state for cascade (from previous iteration)
            cascade_rate: Rate for cascade blending
            action: Optional action tensor for BPTT

        Returns:
            Tuple of (action, action_log_prob, entropy, value, log_probs, prev_hidden)
        """
        ...
        # IMPLEMENT CASCADE MODEL AT ENCODED_OBS LEVEL (AFTER COMPUTATION)
        current_hidden = None  # This will be returned as prev_hidden for next iteration
        
        if 'encoded_obs' in td:
            # This is equivalent to your: Hidden = f.relu(self.fc_hidden(Input))
            current_hidden = td['encoded_obs']  # Shape: [48, 128]
            
            # Apply cascade if we have previous hidden state (equivalent to your QNetwork logic)
            if prev_hidden is not None:
                # This is your: Hidden = cascade_rate*Hidden + (1-cascade_rate)*prev_h2
                cascaded_hidden = cascade_rate * current_hidden + (1 - cascade_rate) * prev_hidden
        ...
        comparison = None
        if 'obs_flattener' in td and 'encoded_obs' in td:
            input_features = td['obs_flattener']  # Equivalent to your Input
            hidden_features = td['encoded_obs']   # Equivalent to your Hidden (after cascade)
            
            fc1_component = self.components['fc1']
            if hasattr(fc1_component, '_net') and len(fc1_component._net) > 0:
                fc1_linear = fc1_component._net[0]
                if hasattr(fc1_linear, 'weight'):
                    # This is equivalent to: Output_comparison = f.relu(f.linear(Hidden, self.fc_hidden.weight.t()))
                    fc1_weight_transposed = fc1_linear.weight.t()
                    reconstructed_input = torch.relu(
                        torch.nn.functional.linear(hidden_features, fc1_weight_transposed)
                    )
                    # This is equivalent to: Comparison = Input - Output_comparison
                    comparison = input_features - reconstructed_input

        # Continue with action network (will use cascaded encoded_obs)
        # This is equivalent to: x = self.actions(Hidden) where Hidden is after cascade
