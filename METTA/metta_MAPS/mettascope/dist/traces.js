...
export function drawTrace(panel) {
        ...
        if (ui.mouseDoubleClick) {
            ...
            if (localMousePos != null) {
                ...
                if (mapX > 0 &&
                    ...
                    ui.mapPanel.focusPos(getAttr(state.selectedGridObject, 'c') * Common.TILE_SIZE, getAttr(state.selectedGridObject, 'r') * Common.TILE_SIZE, Common.DEFAULT_ZOOM_LEVEL);
    ...
    // Draw the background.
    ctx.drawSolidRect(rect.x, rect.y, rect.width, rect.height, 
    ...
    ctx.drawSolidRect(state.step * Common.TRACE_WIDTH, 0, Common.TRACE_WIDTH, fullSize.y(), [0.5, 0.5, 0.5, 0.5] // White with 50% opacity.
    );
    ...
    for (let i = 0; i < state.replay.num_agents; i++) {
        ...
        for (let j = 0; j < state.replay.max_steps; j++) {
            ...
            if (getAttr(agent, 'agent:frozen', j) > 0) {
                ...
                ctx.drawSprite('trace/frozen.png', j * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, i * Common.TRACE_HEIGHT + Common.TRACE_HEIGHT / 2);
            }
            else if (action_success && action != null && action[0] >= 0 && action[0] < state.replay.action_images.length) {
                // Draw the action.
                ctx.drawSprite(state.replay.action_images[action[0]], j * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, i * Common.TRACE_HEIGHT + Common.TRACE_HEIGHT / 2);
            }
            else if (action != null && action[0] >= 0 && action[0] < state.replay.action_images.length) {
                // Draw the invalid action.
                ctx.drawSprite('trace/invalid.png', j * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, i * Common.TRACE_HEIGHT + Common.TRACE_HEIGHT / 2);
            }
            const reward = getAttr(agent, 'reward', j);
            // If there is a reward, draw a coin.
            if (reward > 0) {
                ctx.drawSprite('resources/reward.png', j * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, i * Common.TRACE_HEIGHT + Common.TRACE_HEIGHT - 32, [1.0, 1.0, 1.0, 1.0], 1 / 4);
            }
            ...
            if (state.showResources && j > 0) {
                ...
                for (const [key, [image, color]] of state.replay.resource_inventory) {
                    ...
                    for (let k = 0; k < absGain; k++) {
                        ctx.drawSprite(image, j * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, i * Common.TRACE_HEIGHT + y, color, 1 / 4);
                        y += step;
                    }
                }
            }
