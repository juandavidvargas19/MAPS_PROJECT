import { find, finds, removeChildren, findAttr, onEvent, showMenu, hideMenu, showDropdown, localStorageSetObject, localStorageGetObject, } from './htmlutils.js';
...
class ColumnDefinition {
    constructor(field, isFinal, sortDirection = SortDirection.None, isStepColumn = false) {
        this.field = field;
...
function swapRight(list, element) {
...
function swapLeft(list, element) {
    let index = list.indexOf(element);
    if (index == -1) {
        return;
    }
...
onEvent('click', '#agent-panel .header-cell .dropdown', (target, e) => {
...
function toggleSortDirection(columnField, columnIsFinal) {
...
onEvent('click', '#column-menu .move-right', (target, e) => {
    let { columnField, columnIsFinal } = getFieldInfo(target);
    let column = columns.find((column) => column.field == columnField && column.isFinal == columnIsFinal);
    if (column != null) {
...
onEvent('click', '#column-menu .hide-column', (target, e) => {
    let { columnField, columnIsFinal } = getFieldInfo(target);
...
onEvent('click', '#agent-panel .header-cell', (target, e) => {
    let { columnField, columnIsFinal } = getFieldInfo(target);
    if (columnField != '') {
...
onEvent('click', '#agent-panel .data-cell', (target, e) => {
 ...
 */
onEvent('click', '#new-column-input', (target, e) => {
...
onEvent('input', '#new-column-input', (target, e) => {
    updateAvailableColumns();
});
/** Toggles the column in the columns array based on the field and isFinal. */
function toggleColumn(columnField, columnIsFinal) {
    let found = -1;
    if (columnField != '') {
        for (let i = 0; i < columns.length; i++) {
            if (columns[i].field == columnField && columns[i].isFinal == columnIsFinal) {
 ...
 */
onEvent('click', '#new-column-dropdown .step-check', (target, e) => {
 ...
 * from the columns array.
 */
onEvent('click', '#new-column-dropdown .final-check', (target, e) => {
...
export function updateAvailableColumns() {
    ...
    for (let column of availableColumns) {
        let option = columnOptionTemplate.cloneNode(true);
...
export function updateAgentTable() {
    ...
    for (let columnDef of columns) {
        ...
        let sortIcon = headerCell.querySelector('.sort-icon');
        ...
        for (let i = 0; i < agents.length; i++) {
            let agent = agents[i];
            if (agent != null) {
                let dataCell = dataCellTemplate.cloneNode(true);
                var value;
    ...
    let newColumn = newColumnTemplate.cloneNode(true);
    let headerCell = newColumnHeaderCell.cloneNode(true);
    newColumn.appendChild(headerCell);
    for (let i = 0; i < agents.length; i++) {
        let dataCell = newColumnDataCell.cloneNode(true);
        let agent = agents[i];
        let agentId = getAttr(agent, 'agent_id');
        dataCell.setAttribute('data-agent-id', agentId.toString());
        if (state.selectedGridObject != null && agentId == getAttr(state.selectedGridObject, 'agent_id')) {
