...
import { updateHoverPanel, updateReadout } from './hoverpanels.js';
import { search, searchMatch } from './search.js';
/** Generates a color from an agent ID. */
function colorFromId(agentId) {
...
export function focusMap(x, y, w, h) {
...
export function focusFullMap(panel) {
...
function drawFloor() {
    ...
    ctx.drawSolidRect(-Common.TILE_SIZE / 2, -Common.TILE_SIZE / 2, state.replay.map_size[0] * Common.TILE_SIZE, state.replay.map_size[1] * Common.TILE_SIZE, floorColor);
}
...
function drawWalls() {
    ...
    for (const gridObject of state.replay.grid_objects) {
        ...
        if (e && s && se) {
            ctx.drawSprite('objects/wall.fill.png', x * Common.TILE_SIZE + Common.TILE_SIZE / 2, y * Common.TILE_SIZE + Common.TILE_SIZE / 2 - 42);
        }
    }
}
function drawObject(gridObject) {
    const type = getAttr(gridObject, 'type');
    const typeName = state.replay.object_types[type];
    if (typeName === 'wall') {
    ...
    if (gridObject['agent_id'] !== undefined) {
        ...
        if (colorIdx >= 0 && colorIdx < Common.COLORS.length) {
            ctx.drawSprite(state.replay.object_images[type][2], x * Common.TILE_SIZE, y * Common.TILE_SIZE, Common.COLORS[colorIdx][1]);
        }
        // Draw the item layer.
...
function drawActions() {
    for (const gridObject of state.replay.grid_objects) {
        ...
        if (gridObject['action'] !== undefined) {
            ...
            if (action_success && action != null) {
                ...
                if (action_name == 'attack' && action[1] >= 1 && action[1] <= 9) {
                    ctx.drawSprite('actions/attack' + action[1] + '.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE, [1, 1, 1, 1], 1, rotation);
                }
                else if (action_name == 'attack_nearest') {
                    ctx.drawSprite('actions/attack_nearest.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE, [1, 1, 1, 1], 1, rotation);
                }
                else if (action_name == 'put_recipe_items') {
                    ctx.drawSprite('actions/put_recipe_items.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE, [1, 1, 1, 1], 1, rotation);
                }
                else if (action_name == 'get_output') {
                    ctx.drawSprite('actions/get_output.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE, [1, 1, 1, 1], 1, rotation);
                }
        ...
        if (getAttr(gridObject, 'converting') > 0) {
            ctx.drawSprite('actions/converting.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE - 100, [1, 1, 1, 1], 1, 
            ...
            if (num !== null && num !== undefined && num > 0) {
                for (let i = 0; i < num; i++) {
                    if (useSearch) {
                        ...
                        ctx.drawSprite('effects/halo.png', x * Common.TILE_SIZE + inventoryX - Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2 + 16, [1, 1, 1, 1], 0.25, 0);
                    }
                    ctx.drawSprite(icon, x * Common.TILE_SIZE + inventoryX - Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2 + 16, color, 1 / 8, 0);
                    inventoryX += advanceX;
                }
            }
        }
    ...
    for (const gridObject of state.replay.grid_objects) {
        ...
        if (gridObject['total_reward'] !== undefined) {
            ...
            for (let i = 0; i < totalReward; i++) {
                ctx.save();
                ctx.translate(x * Common.TILE_SIZE + rewardX - Common.TILE_SIZE / 2, y * Common.TILE_SIZE + Common.TILE_SIZE / 2 - 16);
                ctx.scale(1 / 8, 1 / 8);
    ...
    if (state.selectedGridObject != null && state.selectedGridObject.agent_id != null) {
        ...
        if (keyAction != null) {
            ...
            if (state.step == keyActionStep) {
                ctx.drawSprite('actions/thoughts_lightning.png', x * Common.TILE_SIZE + Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2);
            }
            else {
                ctx.drawSprite('actions/thoughts.png', x * Common.TILE_SIZE + Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2);
            }
            // Draw the action icon.
            var iconName = 'actions/icons/' + state.replay.action_names[keyAction[0]] + '.png';
            if (ctx.hasImage(iconName)) {
                ctx.drawSprite(iconName, x * Common.TILE_SIZE + Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2, [1, 1, 1, 1], 1 / 4, 0);
            }
            else {
                ctx.drawSprite('actions/icons/unknown.png', x * Common.TILE_SIZE + Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2, [1, 1, 1, 1], 1 / 4, 0);
            }
...
function drawVisibility() {
    if (state.showVisualRanges || state.showFogOfWar) {
        ...
        function updateVisibilityMap(gridObject) {
            const x = getAttr(gridObject, 'c');
            const y = getAttr(gridObject, 'r');
        ...
        for (let x = 0; x < state.replay.map_size[0]; x++) {
            for (let y = 0; y < state.replay.map_size[1]; y++) {
                if (!visibilityMap.get(x, y)) {
                    ctx.drawSolidRect(x * Common.TILE_SIZE - Common.TILE_SIZE / 2, y * Common.TILE_SIZE - Common.TILE_SIZE / 2, Common.TILE_SIZE, Common.TILE_SIZE, color);
                }
            }
        }
    }
...
function attackGrid(orientation, idx) {
    //                           North\0
    //                       +---+---+---+
    //                       | 7 | 8 | 9 |
    //                       +---+---+---+
...
function drawAttackMode() {
    ...
    if (ui.mouseUp && ui.mouseTargets.includes('#worldmap-panel') && state.showAttackMode) {
        ...
        if (localMousePos != null) {
            gridMousePos = new Vec2f(Math.round(localMousePos.x() / Common.TILE_SIZE), Math.round(localMousePos.y() / Common.TILE_SIZE));
        }
    }
...
function drawInfoLine(panel) {
    ...
    ctx.drawSpriteLine('dash.png', x * Common.TILE_SIZE, y * Common.TILE_SIZE, panelWorldPos.x(), panelWorldPos.y(), 60, [1, 1, 1, 1], 2);
}
/** Draws the world map. */
export function drawMap(panel) {
    ...
    if (ui.mouseTargets.includes('#worldmap-panel')) {
        if (ui.dragging == '' && !state.showAttackMode) {
            ...
            if (localMousePos != null) {
                const gridMousePos = new Vec2f(Math.round(localMousePos.x() / Common.TILE_SIZE), Math.round(localMousePos.y() / Common.TILE_SIZE));
                objectUnderMouse = state.replay.grid_objects.find((obj) => {
        ...
        if (ui.mouseDoubleClick) {
            ...
            if (objectUnderMouse !== undefined) {
                ...
                if (state.selectedGridObject.agent_id !== undefined) {
                    ...
                    ui.tracePanel.focusPos(state.step * Common.TRACE_WIDTH + Common.TRACE_WIDTH / 2, getAttr(state.selectedGridObject, 'agent_id') * Common.TRACE_HEIGHT + Common.TRACE_HEIGHT / 2, Common.DEFAULT_TRACE_ZOOM_LEVEL);
                }
            }
        }
        else {
    ...
    if (search.active) {
        ...
        ctx.drawSolidRect(-Common.TILE_SIZE / 2, -Common.TILE_SIZE / 2, state.replay.map_size[0] * Common.TILE_SIZE, state.replay.map_size[1] * Common.TILE_SIZE, [0, 0, 0, 0.80]);
        drawSelection();
