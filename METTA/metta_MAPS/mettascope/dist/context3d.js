...
class Mesh {
    constructor(name, device, maxQuads = 1024 * 8) {
        ...
        this.scissorRect = [0, 0, 0, 0]; // x, y, width, height
        this.name = name;
        this.device = device;
    ...
    resizeMaxQuads(newMaxQuads) {
        ...
        if (this.vertexBuffer && this.indexBuffer && this.device) {
            ...
            if (currentVertexCount > 0) {
                this.device.queue.writeBuffer(this.vertexBuffer, 0, this.vertexData, 0, currentVertexCount * 8 // 8 floats per vertex
                );
            }
        }
    }
    ...
    drawRectWithTransform(topLeft, bottomLeft, topRight, bottomRight, u0, v0, u1, v1, color = [1, 1, 1, 1]) {
...
export class Context3d {
    ...
    save() {
        ...
        this.transformStack.push(new Mat3f(this.currentTransform.get(0, 0), this.currentTransform.get(0, 1), this.currentTransform.get(0, 2), this.currentTransform.get(1, 0), this.currentTransform.get(1, 1), this.currentTransform.get(1, 2), this.currentTransform.get(2, 0), this.currentTransform.get(2, 1), this.currentTransform.get(2, 2)));
    }
    ...
    async init(atlasJsonUrl, atlasImageUrl) {
        this.dpr = 1.0;
        ...
        this.device.queue.copyExternalImageToTexture({ source, flipY: false }, // Don't flip Y if UVs start top-left.
        { texture: this.atlasTexture }, { width: this.textureSize.x(), height: this.textureSize.y() });
    ...
    drawRect(x, y, width, height, u0, v0, u1, v1, color = [1, 1, 1, 1]) {
        if (!this.ready) {
    ...
    drawImage(imageName, x, y, color = [1, 1, 1, 1]) {
        if (!this.ready) {
            throw new Error('Drawer not initialized');
        }
        this.ensureMeshSelected();
        ...
        this.drawRect(x - m, // Adjust x position by adding margin (from the right).
    ...
    drawSprite(imageName, x, y, color = [1, 1, 1, 1], scale = 1, rotation = 0) {
        if (!this.ready) {
            throw new Error('Drawer not initialized');
        }
        this.ensureMeshSelected();
        ...
        if (scale != 1 || rotation != 0) {
            ...
            this.drawRect(-sw / 2 - m, // Center horizontally with margin adjustment.
            ...
            this.drawRect(x - sw / 2 - m, // Center horizontally with margin adjustment.
    ...
    drawStrokeRect(x, y, width, height, strokeWidth, color) {
    ...
    flush() {
        ...
        device.queue.writeBuffer(this.canvasSizeUniformBuffer, 0, // Buffer offset.
        ...
        if (this.renderPassDescriptor && this.context) {
            ...
            for (const mesh of this.meshes.values()) {
                ...
                if (mesh.scissorEnabled) {
                    ...
                    passEncoder.setScissorRect(clamp(Math.floor(x), 0, w), clamp(Math.floor(y), 0, h), clamp(Math.floor(width), 0, w - x), clamp(Math.floor(height), 0, h - y));
                }
                else {
     ...
     * and end of the line.
     */
    drawSpriteLine(imageName, x0, y0, x1, y1, spacing, color, skipStart = 0, skipEnd = 0) {
