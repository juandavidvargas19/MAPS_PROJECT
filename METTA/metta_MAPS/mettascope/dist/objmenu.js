 ...
 */
import { find, onEvent, showMenu, findAttr } from './htmlutils.js';
...
onEvent('click', '.hover-panel .memory', (target, e) => {
...
onEvent('click', '#object-menu .set-memory-to-0', (target, e) => {
    if (state.ws == null)
        return;
...
onEvent('click', '#object-menu .set-memory-to-1', (target, e) => {
    if (state.ws == null)
        return;
    let agentId = parseInt(findAttr(target, 'data-agent-id'));
    state.ws.send(JSON.stringify({
...
onEvent('click', '#object-menu .set-memory-to-random', (target, e) => {
    if (state.ws == null)
        return;
    let agentId = parseInt(findAttr(target, 'data-agent-id'));
    state.ws.send(JSON.stringify({
...
onEvent('click', '#object-menu .copy-memory', async (target, e) => {
    if (state.ws == null)
        return;
    let agentId = parseInt(findAttr(target, 'data-agent-id'));
...
onEvent('click', '#object-menu .paste-memory', async (target, e) => {
    if (state.ws == null)
        return;
    let agentId = parseInt(findAttr(target, 'data-agent-id'));
    try {
        const clipboardText = await navigator.clipboard.readText();
        const memory = JSON.parse(clipboardText);
        state.ws.send(JSON.stringify({
            type: 'paste_memory',
            agent_id: agentId,
            memory: memory,
