...
let hoverPanel = hoverPanelTemplate.cloneNode(true);
document.body.appendChild(hoverPanel);
findIn(hoverPanel, '.actions').classList.add('hidden');
hoverPanel.classList.add('hidden');
hoverPanel.addEventListener('mousedown', (e) => {
    // Create a new info panel.
    let panel = new HoverPanel(ui.delayedHoverObject);
    panel.div = hoverPanelTemplate.cloneNode(true);
...
export function updateHoverPanel(object) {
...
function updateDom(htmlPanel, object) {
    // Update the readout.
    ...
    for (const key in object) {
        ...
        if ((key.startsWith('inv:') || key.startsWith('agent:inv:')) && value > 0) {
            ...
            let param = paramTemplate.cloneNode(true);
