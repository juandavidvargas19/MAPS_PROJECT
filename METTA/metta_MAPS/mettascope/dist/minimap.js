...
/** Draws the minimap. */
export function drawMiniMap(panel) {
    ...
    if (ui.mouseDown && panel.inside(ui.mousePos)) {
        ...
        const miniMapMousePos = new Vec2f(Math.round(localMousePos.x() / Common.MINI_MAP_TILE_SIZE), Math.round(localMousePos.y() / Common.MINI_MAP_TILE_SIZE));
        ui.mapPanel.panPos = new Vec2f(-miniMapMousePos.x() * Common.TILE_SIZE - (state.replay.map_size[0] * Common.TILE_SIZE) / 2, -miniMapMousePos.y() * Common.TILE_SIZE - (state.replay.map_size[1] * Common.TILE_SIZE) / 2);
    ...
    ctx.drawSolidRect(0, 0, state.replay.map_size[0] * Common.MINI_MAP_TILE_SIZE, state.replay.map_size[1] * Common.MINI_MAP_TILE_SIZE, parseHtmlColor('#E7D4B7'));
    ...
    for (const gridObject of state.replay.grid_objects) {
        ...
        ctx.drawSolidRect(x * Common.MINI_MAP_TILE_SIZE, y * Common.MINI_MAP_TILE_SIZE, Common.MINI_MAP_TILE_SIZE, Common.MINI_MAP_TILE_SIZE, color);
    }
    ...
    for (const gridObject of state.replay.grid_objects) {
        ...
        const typeName = state.replay.object_types[type];
        if (typeName === 'agent') {
            ctx.drawSprite('minimapPip.png', x * Common.MINI_MAP_TILE_SIZE + 1, y * Common.MINI_MAP_TILE_SIZE + 1, [1, 0, 0, 1], 1, 0);
            continue;
        }
    }
    // Draw where the screen is on the minimap.
    const pos = new Vec2f((-ui.mapPanel.panPos.x() / Common.TILE_SIZE) * Common.MINI_MAP_TILE_SIZE, (-ui.mapPanel.panPos.y() / Common.TILE_SIZE) * Common.MINI_MAP_TILE_SIZE);
